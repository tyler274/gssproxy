...
use gssapi_sys::sys::{
    self, OM_uint32, gss_OID, gss_OID_set, gss_buffer_set_t, gss_buffer_t,
    gss_const_key_value_set_t, gss_cred_id_t, gss_name_t,
};
...
use crate::handle::{CredHandle, NameHandle, name_to_local, store_remote_creds};
...
pub unsafe extern "C" fn gssi_acquire_cred_from(
    ...
    unsafe {
        logging::init();
        let _span = tracing::debug_span!("gssi_acquire_cred_from", cred_usage, time_req).entered();
        tracing::debug!("acquire_cred: enter");
        ...
        if maj == COMPLETE {
            ...
            if let Some(rc) = &out.remote
                && !handle::creds_are_equal(in_cred_remote.as_ref(), Some(rc))
            {
...
pub unsafe extern "C" fn gssi_inquire_cred(
    ...
    unsafe {
        ...
        if no_cred {
            let (maj, min) =
                handle::get_def_creds(behavior::get(), None, GSS_C_INITIATE, &mut owned);
            if maj != COMPLETE {
                set_min(minor_status, min);
                return maj;
            }

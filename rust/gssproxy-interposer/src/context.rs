...
use gssapi_sys::sys::{
    self, OM_uint32, gss_OID, gss_buffer_t, gss_channel_bindings_t, gss_cred_id_t, gss_ctx_id_t,
    gss_name_t,
};
...
use crate::handle::{
    CredHandle, CtxHandle, NameHandle, local_to_name, name_to_local, store_remote_creds,
};
use crate::{handle, logging, special};
...
pub unsafe extern "C" fn gssi_init_sec_context(
    ...
    unsafe {
        logging::init();
        let _span = tracing::debug_span!("gssi_init_sec_context").entered();
        tracing::debug!(
            req_flags,
            time_req,
            input_token = logging::buf_len(input_token),
            "init_sec_context: enter"
        );
...
pub unsafe extern "C" fn gssi_accept_sec_context(
    ...
    unsafe {
        logging::init();
        let _span = tracing::debug_span!("gssi_accept_sec_context").entered();
        tracing::debug!(
            input_token = logging::buf_len(input_token_buffer),
            "accept_sec_context: enter"
        );

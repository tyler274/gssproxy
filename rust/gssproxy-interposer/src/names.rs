...
use crate::handle::{name_to_local, NameHandle, OwnedOid};
use crate::special;
...
pub unsafe extern "C" fn gssi_display_name(
    ...
    if !name.local.is_null() {
        ...
        let maj = sys::gss_display_name(&mut min, name.local, output_name_buffer, output_name_type);
...
pub unsafe extern "C" fn gssi_display_name_ext(
    ...
    let mut min: OM_uint32 = 0;
    let maj = sys::gss_display_name_ext(&mut min, name.local, display_as_name_type, display_name);
    set_min(minor_status, min);
    maj
}
